...
The correlation test shows that all the parameters are positively correlated. A higher retention rate will lead to higher conversion and consequently higher revenues. 
...
N_Day_Analysis%>%
  ...
  geom_boxplot()+
  ggtitle("Figure 2 A/B Test for Retention")+
  xlab('days_since_acquisition')+
  ylab('Retention_Rate')+ 
  theme_economist() + 
  scale_colour_economist()
...
Fig 1. shows that test_group_b clearly had a higher Cumulative ARPU in the first few days. This could be a consequence of early buyers. However their is evidence of rapid increase in cumulative ARPU generated from test_group_a and control_group in comparison to test_group_b. Test_group_a in particular performs the best over the 15 day period.  

Fig 2. shows that retention rate is higher for control_group and test_group_a in the early stages. This could be evidence of higher retention rates for groups in which the display of shop was delayed. The fact that in Figure 1 Cumulative ARPU for test_group_a and control_group rises quicker than test_group_b could be a consequence of higher retention rates for the two groups.
...
# probability of purchase before 2nd game
d_fun(2)*100
# probability of purchase between 2nd and 4th game
(d_fun(4)-d_fun(2))*100
# probability of purchase between 4th and 6th game
(d_fun(6)-d_fun(4))*100

# probability of purchase between 6th and 8th game
(d_fun(8)-d_fun(6))*100
# probability of purchase between 8th and 10th game
(d_fun(10)-d_fun(8))*100

```
The probability of purchasing a product between 1st game and 4th game is:  14%
The probability of purchasing a product between 1st game and 4th game is:  
