...
Figure 3 shows that retention rate drops rapidly during the period of analysis for all groups in general. This denotes that the period of analysis that is 15 days can be assumed to be the overall lifecycle for most users with minimal revenue generation post that for a specific acquisition day.The graph also shows that retention rate is higher for control_group and test_group_a in the early stages. This could be evidence of higher retention rates for groups in which the display of shop was delayed.  
...
The distribution shows that that most users tend to purchase a product between the 5th game and 25th game. The average number of games played prior to first purchase is approx. 14. There is a spike in number of users purchasing a product after 1st, 2nd and 4th game. This could be potentially due to display of shop for test groups at these junctures. Purchase of  a product peaks post the 8th game.However by splitting the distribution study for each group more insight on the early purchasing activity can be garnered.  

```{r distribution matches individual,echo=FALSE, message=FALSE, warning=FALSE}
dist_plot+
  facet_grid(rows=vars(abTestGroup))
```
  
In general it can be seen that when the shop is displayed there is high buying activity. This activity can be quantified by the emperical Cumulative Distribution Function (eCDF).  
...
```{r eCDF eval individual, echo=FALSE,message=FALSE, warning=FALSE}
